...
class ErpApiLogCheck extends Check
{
    ...
    {
        $fileConfig = config('amplify.developer.log_erp_api', false);

        $dbConfig = null;

        if($entry = SystemConfiguration::whereName('developer')->whereOption('log_erp_api')->first()) {
        ...
        return $this->expected === $actual
            ? $result->ok()
            : $result->failed("The ERP api log was expected to be `{$this->convertToWord((bool) $this->expected)}`, but actually was `{$this->convertToWord((bool) $actual)}`");
    }
