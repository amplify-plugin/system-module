...
class DraftOrderReceivedJob implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        foreach ($this->eventInfo?->eventActions ?? [] as $eventAction) {
            if ($eventAction->eventTemplate->notification_type == 'emailable') {
                $this->emailService->sendDraftOrderDetailsEmailToCustomer($eventAction, $order, $customer);
            }

            if ($eventAction->eventTemplate->notification_type == 'messageable') {
                $this->messageService->sendDraftOrderDetailsMessageToCustomer($eventAction->eventTemplate, $order, $customer);
            }
        }
    }
}
