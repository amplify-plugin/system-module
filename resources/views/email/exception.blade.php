...
<head>
    ...
    <title> {{ config('app.name') }} {{ $content['title'] ?? 'ErrorException' }} Notification</title>
    <style>
        body {
                ...
                <th>
                    <h3 class="trace-class">
                        ...
                        <span class="exception_title">
                            <span title="{{ $content['title'] ?? 'ErrorException' }}">
                                {{ $content['title'] ?? 'ErrorException' }}
                            </span>
                        </span>
                    </h3>
            ...
            <table class="trace-details">
                ...
                <tr>
                    <th>
                        <h3 class="trace-class">
                            ...
                            <span class="exception_title">
                            <span title="{{ get_class($content['previous']) }}">
                                {{ get_class($content['previous']) }}
                            </span>
                        </span>
                        </h3>
                ...
                <tr>
                    ...
                    <span class="block trace-file-path">
                        in <span
                                title="{{ $content['previous']->getFile()}}">
                            <strong>{{ $content['previous']->getFile()}}</strong>
                            line {{ $content['previous']->getLine()}}
                        </span>
                    </span>
                    </td>
                </tr>
                @foreach(($content['previous']?->getTrace() ?? []) as $value)
                    <tr>
                        <td>
                            at <span class="trace-class"><span
                                        title="{{ $value['class'] ?? '' }}">{{ basename($value['class'] ?? '') }}</span></span><span
