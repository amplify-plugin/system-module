...
class MicrosoftGraphMailProvider extends ServiceProvider
{
    public function boot(): void
    {
        Mail::extend('microsoft-graph', function (array $config): MicrosoftGraphTransport {
            throw_if(
                ...
                new \Error('Configuration key from.address for microsoft-graph mailer is missing.')
            );

            $accessTokenTtl = $config['access_token_ttl'] ?? 3000;
            if (! is_int($accessTokenTtl)) {
                throw new \Error("Configuration key access_token_ttl for microsoft-graph mailer has invalid value: ".var_export($accessTokenTtl, true).".");
            }
    ...
    protected function requireConfigString(array $config, string $key): string
    {
        if (! array_key_exists($key, $config)) {
            throw new \Error("Configuration key {$key} for microsoft-graph mailer is missing.");
        }

        $value = $config[$key];
        if (! is_string($value) || $value === '') {
            $invalidValue = var_export($value, true);
            throw new \Error("Configuration key {$key} for microsoft-graph mailer has invalid value: {$invalidValue}.");
        }
